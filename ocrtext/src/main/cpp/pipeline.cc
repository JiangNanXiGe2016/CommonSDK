...
bool Pipeline::Process_val(int inTextureId, int outTextureId, int textureWidth,
    ...
    LOGD("debug===Process_val: %d", 1111111);
    ...
    CreateRGBAImageFromGLFBOTexture(textureWidth, textureHeight, &rgbaImage,
                                    &readGLFBOTime);
    LOGD("debug===rgbaImage: %d %d", rgbaImage.cols, rgbaImage.rows);
    LOGD("debug===Process_val: %d", 222222);
    // change to 3-channel
    cv::Mat bgrImage;
    cv::cvtColor(rgbaImage, bgrImage, cv::COLOR_RGBA2BGR);

    LOGD("debug===Process_val: %d", 333333);
    LOGD("debug===bgrImage: %d %d", bgrImage.cols, bgrImage.rows);
    cv::Mat bgrImage_resize;
    cv::resize(bgrImage, bgrImage_resize, cv::Size(width, height));

    LOGD("debug===Process_val: %d", 444444);
    LOGD("debug===bgrImage_resize: %d %d", bgrImage_resize.cols, bgrImage_resize.rows);
    ...
    for (int i = boxes.size() - 1; i >= 0; i--) {
        ...
        LOGD("debug=== text: %s %f", res.first.c_str(), res.second);
...
std::vector<std::pair<std::string, float>> Pipeline::Process_Single_Img(const cv::Mat bgrImage) {
    int height = 448;
    int width = 448;
    // change to 3-channel
//    cv::Mat bgrImage;
//    cv::cvtColor(rgaImage, bgrImage, cv::COLOR_RGBA2BGR);
    LOGD("Process_Single_Img===bgrImage ocr: %d %d",bgrImage.cols,bgrImage.rows);

    cv::Mat bgrImage_resize;
    cv::resize(bgrImage, bgrImage_resize, cv::Size(width, height));
    LOGD("Process_Single_Img===bgrImage_resize ocr: %d %d",bgrImage_resize.cols,bgrImage_resize.rows);
    LOGD("Process_Single_Img===native ocr: %d",3333333);
    int use_direction_classify = int(Config_["use_direction_classify"]);
    cv::Mat srcimg;
    bgrImage_resize.copyTo(srcimg);
    // Stage1: rec
    ...
    auto boxes =
            detPredictor_->Predict(srcimg, Config_, nullptr, nullptr, nullptr);
    LOGD("Process_Single_Img===native ocr: %d",44444444);
    std::vector<float> mean = {0.5f, 0.5f, 0.5f};
    std::vector<float> scale = {1 / 0.5f, 1 / 0.5f, 1 / 0.5f};
    cv::Mat img;
    bgrImage_resize.copyTo(img);
    ...
    std::vector<std::string> rec_text;
    std::vector<float> rec_text_score;
    std::vector<std::pair<std::string, float>> result_vector;
    LOGD("Process_Single_Img===boxes: %d", boxes.size());
    for (int i = boxes.size() - 1; i >= 0; i--) {
        crop_img = GetRotateCropImage(img, boxes[i]);
        if (use_direction_classify >= 1) {
            crop_img =
        ...
        LOGD("Process_Single_Img=== text: %s %f", res.first.c_str(), res.second);
        rec_text.push_back(res.first);
        rec_text_score.push_back(res.second);
        result_vector.push_back(std::make_pair(res.first, res.second));
    }
